...
class CPAH(nn.Module):
    def __init__(self, image_dim, text_dim, hidden_dim, hash_dim, label_dim, pretrain_model):
        ...
        self.vgg = image_net(pretrain_model).cuda()
    ...
    def img_model(self, i):
        i = self.vgg(i).squeeze()
        if i.ndim == 1:
            i = i.unsqueeze(0)
        f_i = self.image_module(i)
        f_i = self.hash_module['img'](f_i)
        return f_i
    
    def txt_model(self, t):
        f_t = self.text_module(t)
        return self.hash_module['txt'](f_t)
