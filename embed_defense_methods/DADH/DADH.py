...
class GEN(torch.nn.Module):
    ...
    def generate_img_code(self, i):
        i = self.cnn_f(i)
        f_i = self.image_module(i.squeeze().unsqueeze(0))
    ...
    def img_model(self, i):
        ...
        code = self.hash_module['image'](f_i).reshape(-1, self.output_dim)
        return code

    def txt_model(self, t):
        f_t = self.text_module(t)
        code = self.hash_module['text'](f_t).reshape(-1, self.output_dim)
        return code
