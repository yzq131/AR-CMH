import torch
import torch.nn as nn
import scipy.io as scio


class Defense_Model(nn.Module):
    def __init__(self, method, dataset, bit, attacked_models_path, dataset_path):
        super(Defense_Model, self).__init__()
        self.method = method
        self.dataset = dataset
        self.bit = bit
        vgg_path = dataset_path + 'imagenet-vgg-f.mat'
        if self.dataset == 'FLICKR':
            tag_dim = 1386
            num_label = 24
        if self.dataset == 'COCO':
            tag_dim = 1024
            num_label = 80
        if self.dataset == 'NUS':
            tag_dim = 1000
            num_label = 21

        if self.method == 'CPAH':
            CPAH_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(self.bit) + '/CPAH.pth'
            image_dim = 4096
            hidden_dim = 512
            from embed_defense_methods.CPAH.CNN_F import image_net
            from embed_defense_methods.CPAH.CPAH import CPAH
            pretrain_model = scio.loadmat(vgg_path)
            self.vgg = image_net(pretrain_model)
            self.model = CPAH(image_dim, tag_dim, hidden_dim, self.bit, num_label, pretrain_model)
            self.model.load(CPAH_path)
            self.vgg.cuda().eval()
            self.model.cuda().eval()

        if self.method == 'DAPH':
            load_img_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(
                self.bit) + '/image_model.pth'
            load_txt_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(
                self.bit) + '/text_model.pth'
            from embed_defense_methods.DAPH.DAPH import HASH_Net, TxtNet
            pretrain_model = scio.loadmat(vgg_path)
            self.image_hashing_model = HASH_Net('alexnet', self.bit, True, pretrain_model)
            self.text_hashing_model = TxtNet(tag_dim, self.bit)
            self.image_hashing_model.load(load_img_path)
            self.text_hashing_model.load(load_txt_path)
            self.image_hashing_model.cuda().eval()
            self.text_hashing_model.cuda().eval()

        if self.method == 'DADH':
            DADH_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(self.bit) + '/DADH.pth'
            dropout = False
            image_dim = 4096
            hidden_dim = 8192
            from embed_defense_methods.DADH.DADH import GEN
            pretrain_model = scio.loadmat(vgg_path)
            self.generator = GEN(dropout, image_dim, tag_dim, hidden_dim, self.bit, pretrain_model=pretrain_model)
            self.generator.load(DADH_path)
            self.generator.cuda().eval()
            
        if self.method == 'DCPH':
            load_img_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(
                self.bit) + '/image_model.pth'
            load_txt_path = attacked_models_path + str(self.method) + '_' + self.dataset + '_' + str(
                self.bit) + '/text_model.pth'
            from embed_defense_methods.DCPH.DCPH import HASH_Net, TxtNet
            pretrain_model = scio.loadmat(vgg_path)
            self.image_hashing_model = HASH_Net('alexnet', self.bit, True, pretrain_model)
            self.text_hashing_model = TxtNet(tag_dim, self.bit)
            self.image_hashing_model.load(load_img_path)
            self.text_hashing_model.load(load_txt_path)
            self.image_hashing_model.cuda().eval()
            self.text_hashing_model.cuda().eval()

    def generate_image_feature(self, data_images):
        num_data = data_images.size(0)
        B = torch.zeros(num_data, self.bit)
        if self.method == 'DAPH':
            for i in range(num_data):
                output = self.image_hashing_model(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DCPH':
            for i in range(num_data):
                output = self.image_hashing_model(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'CPAH':
            for i in range(num_data):
                temp = (data_images[i].type(torch.float).unsqueeze(0).cuda())
                output = self.model.generate_img_code(temp.squeeze().unsqueeze(0))
                B[i, :] = output.data
        if self.method == 'DADH':
            for i in range(num_data):
                output = self.generator.generate_img_code(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.cpu().data
        return B

    def generate_text_feature(self, data_texts):
        num_data = data_texts.size(0)
        B = torch.zeros(num_data, self.bit)
        if self.method == 'DAPH':
            for i in range(num_data):
                output = self.text_hashing_model(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DCPH':
            for i in range(num_data):
                output = self.text_hashing_model(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'CPAH':
            for i in range(num_data):
                output = self.model.generate_txt_code(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DADH':
            for i in range(num_data):
                output = self.generator.generate_txt_code(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.cpu().data
        return B

    def generate_image_hashcode(self, data_images):
        num_data = data_images.size(0)
        B = torch.zeros(num_data, self.bit)
        if self.method == 'DAPH':
            for i in range(num_data):
                output = self.image_hashing_model(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DCPH':
            for i in range(num_data):
                output = self.image_hashing_model(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'CPAH':
            for i in range(num_data):
                temp = (data_images[i].type(torch.float).unsqueeze(0).cuda())
                output = self.model.generate_img_code(temp.squeeze().unsqueeze(0))
                B[i, :] = output.data
        if self.method == 'DADH':
            for i in range(num_data):
                output = self.generator.generate_img_code(data_images[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.cpu().data
        return torch.sign(B)

    def generate_text_hashcode(self, data_texts):
        num_data = data_texts.size(0)
        B = torch.zeros(num_data, self.bit)
        if self.method == 'DAPH':
            for i in range(num_data):
                output = self.text_hashing_model(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DCPH':
            for i in range(num_data):
                output = self.text_hashing_model(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'CPAH':
            for i in range(num_data):
                output = self.model.generate_txt_code(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.data
        if self.method == 'DADH':
            for i in range(num_data):
                output = self.generator.generate_txt_code(data_texts[i].type(torch.float).unsqueeze(0).cuda())
                B[i, :] = output.cpu().data
        return torch.sign(B)

    def image_model(self, data_images):
        if self.method == 'DAPH':
            output = self.image_hashing_model(data_images)
        if self.method == 'DCPH':
            output = self.image_hashing_model(data_images)
        if self.method == 'CPAH':
            data_images = (data_images)
            output = self.model.img_model(data_images)
        if self.method == 'DADH':
            output = self.generator.img_model(data_images)
        return output

    def text_model(self, data_texts):
        if self.method == 'DAPH':
            data_texts = data_texts.type(torch.float)
            output = self.text_hashing_model(data_texts)
        if self.method == 'DCPH':
            data_texts = data_texts.type(torch.float)
            output = self.text_hashing_model(data_texts)
        if self.method == 'CPAH':
            data_texts = data_texts.type(torch.float)
            output = self.model.txt_model(data_texts)
        if self.method == 'DADH':
            data_texts = data_texts.type(torch.float)
            output = self.generator.txt_model(data_texts)
        return output
